...
def invoke_structured_strict(
    structured_llm: Any | None,
    prompt: Any,
    render: Callable[[T], str],
    agent_name: str,
) -> str:
    """Invoke a schema-bound LLM and fail rather than weakening the contract.

    Evidence-normalization nodes feed downstream investment decisions, so a
    free-text retry would make required source/date/missing-data fields
    optional exactly when the first attempt was unreliable.
    """
