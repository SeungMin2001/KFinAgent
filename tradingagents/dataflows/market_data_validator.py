...
def _verified_rows(symbol: str, curr_date: str) -> pd.DataFrame:
    ...
    if vendor == "kis":
        # The KIS provider returns lowercase columns indexed by date.  Request
        # enough prior bars for the fixed 200-day snapshot indicator set, then
        # normalize to the existing validator's capitalized schema.
        from tradingagents.dataflows.kis import get_kis_ohlcv_frame

        end = pd.Timestamp(curr_date).date()
        start = end - timedelta(days=420)
        data = get_kis_ohlcv_frame(symbol, start.isoformat(), end.isoformat()).reset_index()
        data = data.rename(
            columns={
                "date": "Date",
                "open": "Open",
...
def build_verified_market_snapshot(
    ...
    for name in selected:
        ...
        except Exception as exc:  # noqa: BLE001 - strict verification must surface provider/library errors
            raise RuntimeError(f"Failed to calculate required indicator {name}") from exc
    ...
    lines = [
        ...
        f"- Verified OHLCV bars loaded: {len(df)}",
