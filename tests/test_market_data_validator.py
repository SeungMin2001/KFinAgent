...
class TestVerifiedSnapshot:
    ...
    def test_required_indicator_failure_is_fatal(self, monkeypatch):
        monkeypatch.setattr(validator, "load_ohlcv", lambda s, d: _sample_ohlcv())
        with pytest.raises(RuntimeError, match="required indicator definitely_invalid"):
            validator.build_verified_market_snapshot(
                "COF", "2026-05-20", indicators=["definitely_invalid"]
            )
