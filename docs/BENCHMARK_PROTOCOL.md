...
기본 `korean_dart_compact=True`: 원본 ZIP과 추출 텍스트를 artifacts/dart_sources에
SHA256 이름으로 보존한다. 연결·별도 전체 재무제표 API의 모든 행을 열 이름/값 배열로
전달하고, 접수번호·기업·회계연도·보고서코드가 선택한 과거 공시와 일치해야 한다.
분기 손익과 누적 손익, 통화, 비교기간은 API 필드를 보존한다. 숫자/비율을 추정하지 않는다.
원문은 키워드와 주변 2줄 후보에서 문서 전반에 걸쳐 공시당 12,000자 예산으로 발췌한다.
각 줄 번호·선택 정책·생략량을 표시한다. 중요한 내용을 놓칠 수 있고 표 머리글이 빠질 수
있으므로 금액·기간은 구조화 행을 우선한다. 원문 전체 검토와 동등하다고 주장하지 않는다.
원문 전체 방식은 `korean_dart_compact=False`로 명시할 수 있다.
공식 필드 정의: [OpenDART 전체 재무제표](https://opendart.fss.or.kr/guide/detail.do?apiGrpCd=DS003&apiId=2019020).
...
전체 비교는 RunPod 및 KIS/DART/FRED/ECOS/LLM API가 필요하다. 그래프 호출 상한은 기본 120회이며,
응답별 출력은 기본 4,096 토큰으로 제한한다. 이는 총 입력/출력 토큰이나 요금 상한이 아니다.
검증 근거가 판단당 300,000자를 넘으면 유료 토론 전에 중단하며, 검토 후에만
`--max-evidence-chars`를 명시적으로 높인다. 문자 수 역시 정확한 토큰/요금 상한은 아니다.
실패 후에는 새 디렉터리에서 재실행한다. 실패한 실행의 부분 성과를 leaderboard로 만들지 않는다.

비용·초기 보유·배분정책이 달라지면 후속 계좌와 Agent 입력도 달라진다. 기존 판단을
다른 조건에 그대로 재사용한 결과를 계좌 반영 Agent 성과로 취급하지 않는다.
현재 전체 suite는 조건마다 판단을 새로 생성하며, 비용 축소를 위한 판단 재사용은 구현하지 않았다.
